...
module.exports =  (baseConfig, {dev, target}, vendor) => {
  if (typeof vendor !== 'object' && vendor.constructor !== Array) {
  ...
  if (process.env.RAZZLE_VENDOR_MANIFEST) {
    const publicPath = 'http://' + (process.env.HOST || 'localhost') + ':' + process.env.PORT_DEV + '/';
    ...
    appConfig.plugins.push(
      new webpack.WatchIgnorePlugin([paths.appBuild + '/' + process.env.RAZZLE_VENDOR_MANIFEST]),
    )
  }
  return appConfig;
};
